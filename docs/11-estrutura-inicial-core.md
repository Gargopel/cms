...
- `app/Core/Extensions/Hooks`: DTOs e registro central dos primeiros pontos de extensao publicos do core, incluindo slots de tema
...
- `app/Core/Extensions/Settings`: catalogo, leitura e persistencia centralizada de settings declarados por plugins elegiveis
...
- `plugins/Blog`: segundo plugin oficial real, usado como exemplo de plugin editorial com permisses, hooks administrativos, migrations operacionais, rotas publicas e fallback por tema
...
- resolver catalogos de settings declarados por plugins validos e instalados
- persistir e ler settings de plugin com fallback seguro em grupos `plugin:<slug>`
...
- oferecer uma base segura e reutilizavel de media manager para uso futuro por plugins oficiais
- permitir que plugins oficiais reutilizem a biblioteca central de midia para campos simples como imagem destacada, sem duplicar storage ou upload
...
- `app/Core/Themes`: manager do tema ativo, elegibilidade de ativacao, resolvedor de views com fallback e renderer de slots de layout
- `app/Core/Admin/Http/Controllers`: dashboard, extensoes, manutencao, usuarios, cargos, permissoes, settings, auditoria e health
- `app/Core/Admin/Http/Controllers`: dashboard, extensoes, temas, manutencao, mídia, usuarios, cargos, permissoes, settings, settings de plugin, auditoria e health
...
- `resources/views/front/slots`: wrappers fallback para slots do frontend quando o tema ativo nao fornece view propria
...
- `plugins/Forms/resources/views/front`: fallback publico do plugin oficial Forms
- `plugins/Forms/routes`: rotas administrativas e publicas do plugin oficial Forms
- `plugins/Forms/database/migrations`: persistencia propria do plugin oficial Forms
...
- media manager mais rico com transformação, folders complexos ou integrações externas
- settings de plugin agora existem em camada minima e explicita; settings de tema continuam fora deste escopo
