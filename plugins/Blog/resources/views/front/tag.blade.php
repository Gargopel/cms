...
<head>
    ...
    @else
        <title>{{ $tag->name }} - {{ $blogTitle }} - {{ config('app.name') }}</title>
    @endif
    <style>
        :root {
            --bg: #07111f;
...
<body>
    <main class="shell">
        <header class="hero">
            ...
            <h1>{{ $tag->name }}</h1>
            <p class="subtle">{{ $tag->description ?: 'Posts publicados filtrados por tag editorial.' }}</p>
        </header>
        ...
        <section class="grid">
            ...
            @empty
                <article class="card">
                    <h2>No published posts for this tag.</h2>
                    <p class="subtle">A tag ainda nao possui posts publicados nesta instalacao.</p>
                </article>
            @endforelse
        </section>
    </main>
